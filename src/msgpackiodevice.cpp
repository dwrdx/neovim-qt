...
MsgpackIODevice::MsgpackIODevice(QIODevice *dev, QObject *parent)
    : QObject(parent), m_reqid(0), m_dev(dev), m_encoding(0), m_reqHandler(0),
      m_error(NoError) {
...
/** The encoding used by the MsgpackIODevice::encode and MsgpackIODevice::decode
 * methods @see setEncoding */
...
void MsgpackIODevice::dataAvailableStdin(const QByteArray &data) {
  if ((quint64)data.length() > msgpack_unpacker_buffer_capacity(&m_uk)) {
    setError(InvalidDevice, tr("Error when reading from stdin, BUG(buffered "
                               "data exceeds capaciy)"));
    return;
...
void MsgpackIODevice::dispatch(msgpack_object &req) {
  ...
  case 1:
    if (req.via.array.ptr[1].type != MSGPACK_OBJECT_POSITIVE_INTEGER) {
      qDebug()
          << "Received Invalid response: msg id MUST be a positive integer";
      return;
    }
...
bool MsgpackIODevice::decodeMsgpack(const msgpack_object &in, QByteArray &out) {
  if (in.type == MSGPACK_OBJECT_BIN) {
    ...
    qWarning() << "Attempting to decode as QByteArray when type is" << in.type
               << in;
    out = QByteArray();
    return true;
  }
...
void MsgpackIODevice::send(const QList<QByteArray> &list) {
  ...
  foreach (const QByteArray &elem, list) { send(elem); }
}
...
void MsgpackIODevice::send(const QVariant &var) {
  ...
  case QMetaType::QStringList:
    ...
    foreach (const QVariant &elem, var.toList()) { send(elem); }
    break;
...
QByteArray MsgpackIODevice::encode(const QString &str) {
  if (m_encoding) {
    ...
    qWarning() << "Encoding String into MsgpackIODevice without an encoding "
                  "(defaulting to utf8)";
    return str.toUtf8();
  }
}
...
QString MsgpackIODevice::decode(const QByteArray &data) {
  if (m_encoding) {
    ...
    qWarning() << "Decoding String from MsgpackIODevice without an encoding "
                  "(defaulting to utf8)";
